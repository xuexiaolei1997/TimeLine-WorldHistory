...
def create_period(period: PeriodCreate, db: Session = Depends(db_manager.get_db)):
    repo = PeriodRepository(db)
    return repo.create(period.dict())

@router.get("/", response_model=List[Period])
def read_periods(skip: int = 0, limit: int = 100, db: Session = Depends(db_manager.get_db)):
    repo = PeriodRepository(db)
    return repo.list(skip=skip, limit=limit)

@router.get("/{period_id}", response_model=Period)
@handle_app_exceptions
def read_period(period_id: int, db: Session = Depends(db_manager.get_db)):
    repo = PeriodRepository(db)
    return repo.get(period_id)

@router.put("/{period_id}", response_model=Period)
@handle_app_exceptions
def update_period(period_id: int, period: PeriodUpdate, db: Session = Depends(db_manager.get_db)):
    repo = PeriodRepository(db)
    return repo.update(period_id, period.dict(exclude_unset=True))

@router.delete("/{period_id}")
@handle_app_exceptions
def delete_period(period_id: int, db: Session = Depends(db_manager.get_db)):
    repo = PeriodRepository(db)
